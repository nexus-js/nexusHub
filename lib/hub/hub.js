...
Hub.prototype.transmit = function(nickname, toWhom, data) {
    ...
    where.forEach(function(destination) {
        ...
        if (destination == "all") socket.broadcast.emit(message, data);

        // Individual Discrete Clients
        if (hub.discreteClientCheck(destination)) {
            hub.io.to(hub[destination].id).emit(message, data);
        } else {
            hub.log(`destination ${destination} does not exist`);
        }

        // TODO: Delete if the above discrete client list works.
        // if (destination == "display") {
        //     if (hub.display.id != null) {
        //         hub.io.to(hub.display.id).emit(message, data);
        //     }
        // }
        // if (destination == "controller") {
        //     if (hub.controller.id != null) {
        //         hub.io.to(hub.controller.id).emit(message, data);
        //     }
        // }
        // if (destination == "audio") {
        //     if (hub.audio.id != null) {
        //         hub.io.to(hub.audio.id).emit(message, data);
        //     }
        // }
        // if (destination == "max") {
        //     if (hub.max.id != null) {
        //         hub.io.to(hub.max.id).emit(message, data);
        //     }
        // }
...
hub.prototype.discreteClientCheck = function(whom) {
    return hub.discreteClients[whom].id ? 1 : 0;
}
